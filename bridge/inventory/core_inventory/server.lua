...
gg.inventory.canCarryitem = function(src, data)
    return exports.core_inventory:canCarry(src, data.item, data.count or 1, data.metadata or nil)
end
...
gg.inventory.addItem = function(src, data)
    ...
    local success = exports.core_inventory:addItem(src, data.item, data.count, data.slot, data.metadata)
    if not success then 
        return success
    end
...
gg.inventory.removeItem = function(src, data)
    ...
    local success = exports.core_inventory:removeItem(src, data.item, data.count, data.slot)
    if not success then
