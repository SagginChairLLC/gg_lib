    ...
    for i = 1, 50 do
        if NetworkDoesNetworkIdExist(netid) and NetworkDoesEntityExistWithNetworkId(netid) then
            ...
            if not NetworkHasControlOfNetworkId(netid) then
                gg.print.debug("Attempt", i, "requesting control of", netid)
                NetworkRequestControlOfNetworkId(netid)
            else
                gg.print.debug("Got control of netId", netid, "-> entity", entity)
                break
            end
        else
            gg.print.debug("NetId", netid, "does not exist (attempt", i, ")")
        end
        Wait(0)
    end
